...
const (
	RedeemModeAPI         = "api"
	RedeemModeExternalURL = "external_url"
	RedeemModeManual      = "manual"
)
...
type CardSecretBatch struct {
	...
	RechargeProvider    string         `gorm:"type:varchar(50);index" json:"recharge_provider"`      // 兑换供应商（lyxazy/aidone 等）
	RechargeProductType string         `gorm:"type:varchar(50);index" json:"recharge_product_type"`  // 兑换产品类型（chatgpt_plus/gemini 等）
	RedeemMode          string         `gorm:"type:varchar(30);index" json:"redeem_mode"`            // 兑换模式（api/external_url/manual）
	RedeemURL           string         `gorm:"type:varchar(500)" json:"redeem_url"`                  // 外部兑换地址
